...
<div style="margin-right: -12px;">						
		<input id="fav-button" type="submit" class="form-btn" style="float: right;margin-right: 30px;" onclick="return false;" value="Zu den Favoriten">
</div>
	...
	jQuery( function() {
	    ...
	    jQuery( "#datum-select" ).datepicker({
		    ...
		    onSelect: function(dateText) {
				...
				url+=params;
				url+='<?php echo '&beraterId='.$package->beraterId.'&date=';?>'+dateText;
	    ...
	    jQuery( "#fav-button" ).button().on( "click", function() {
		    	var url = '<?php echo $_SERVER['PHP_SELF'];?>';
				
	        	var params = '?task=saveFavorite&format=raw';				

				url+=params;
				url+='&packageId=<?php echo $package->id;?>';
	
		 			jQuery.ajax({
		 				  url: url,
						  context: document.body
						}).done(function(content) {
							alert("In den Favoriten hinterlegt!");
						});
		    });
